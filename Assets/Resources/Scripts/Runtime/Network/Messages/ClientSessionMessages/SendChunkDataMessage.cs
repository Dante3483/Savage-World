        ...
        protected override void ReadData()
        {
            ...
            for (int x = startX; x < startX + chunkSize; x++)
            {
                for (int y = startY; y < startY + chunkSize;)
                {
                    ...
                    {
                        worldDataManager.SetBlockData(x, y, block);
                        worldDataManager.SetWallData(x, y, wall);
                        worldDataManager.SetLiquidData(x, y, liquid, flowValue);
                        worldDataManager.SetTileId(x, y, tileId);
                    }
                }
            }
        }
            ...
            for (int x = startX; x < startX + chunkSize; x++)
            {
                for (int y = startY; y < startY + chunkSize; y++)
                {
                    ...
                    while (gameManager.IsInMapRange(x, y + iterator))
                    {
                        ...
                        byte nextLiquidId = worldDataManager.GetLiquidId(x, y);
                        float nextFlowValue = worldDataManager.GetFlowValue(x, y + iterator);
                        byte nextTileId = worldDataManager.GetTileId(x, y);
                        BlockTypes nextType = worldDataManager.GetBlockType(x, y);
                    ...
                    WrtieBlockData(flags, blockId, wallId, liquidId, flowValue, tileId, type, countOfSameObject);
                    y += countOfSameObject;
                }
            }
        }

        private void WrtieBlockData(byte flags, ushort blockId, ushort wallId, byte liquidId, float flowValue, byte tileId, BlockTypes type, int countOfSameObject)
        {
            _writer.Write(flags);
